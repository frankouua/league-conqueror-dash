      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="hidden lg:flex items-center justify-center gap-2 mb-12">
          {journeyStages.map((stage, index) => (
            <div key={stage.id} className="flex items-center">
              <div className={`flex items-center gap-2 px-4 py-2 rounded-full bg-gradient-to-r ${stage.color} text-white font-medium`}>
        ...
        <div className="grid gap-6">
          {journeyStages.map((stage, index) => (
            <Card 
              key={stage.id} 
              className={`${stage.bgColor} ${stage.borderColor} border-2 overflow-hidden transition-all hover:shadow-lg`}
            >
              <CardHeader className="pb-4">
                <div className="flex flex-col md:flex-row md:items-center gap-4">
                  {/* Stage Number & Icon */}
                  <div className={`flex items-center justify-center w-16 h-16 rounded-2xl bg-gradient-to-br ${stage.color} text-white shadow-lg`}>
              ...
              <CardContent>
                <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-3">
                  {stage.actions.map((action, actionIndex) => (
                    <div 
                      key={actionIndex}
                      className="flex items-start gap-2 p-3 rounded-lg bg-background/60 backdrop-blur-sm"
                    >
                      <CheckCircle2 className={`h-5 w-5 mt-0.5 flex-shrink-0 text-${stage.color.split('-')[1]}-500`} style={{ color: `var(--${stage.color.split('-')[1]}-500, currentColor)` }} />
        ...
        <Card className="mt-8 bg-primary/5 border-primary/20">
          ...
          <CardContent>
            <p className="text-foreground">
              Cada interação com o paciente é uma oportunidade de gerar pontos! 
              Lembre-se: <strong>NPS com citação do seu nome vale bônus extra</strong>, 
